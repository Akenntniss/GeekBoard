...
window.RepairModal = window.RepairModal || {
    ...
    loadRepairDetails(repairId) {
        console.log('🔄 [RepairModal] loadRepairDetails appelé pour réparation:', repairId);
    ...
    showModal(repairId) {
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                if (!data.repair) {
                    throw new Error('Données de réparation manquantes dans la réponse');
                }
            ...
            .catch(error => {
                ...
                // S'assurer que le loader est caché en cas d'erreur
                this.hideLoader();
    ...
    renderRepairDetails(data) {
        ...
        // Le loader est déjà caché dans showModal, mais on s'assure qu'il est bien caché
        this.hideLoader();
     ...
     */
    showLoader() {
        console.log('📊 [RepairModal] showLoader appelé');
        // Assurer la présence des éléments même si init a été fait tôt
        if (!this.elements.loader) {
            this.elements.loader = document.getElementById('repairDetailsLoader');
            console.log('🔍 [RepairModal] Loader trouvé:', !!this.elements.loader);
        }
        if (!this.elements.detailsContainer) {
            this.elements.detailsContainer = document.getElementById('repairDetailsContent');
            console.log('🔍 [RepairModal] Content trouvé:', !!this.elements.detailsContainer);
        }
        if (this.elements.loader) {
            this.elements.loader.style.display = 'block';
            console.log('✅ [RepairModal] Loader affiché');
        } else {
            console.error('❌ [RepairModal] Loader non trouvé!');
        }
        if (this.elements.detailsContainer) {
            this.elements.detailsContainer.style.display = 'none';
            console.log('✅ [RepairModal] Content caché');
        } else {
            console.error('❌ [RepairModal] Content non trouvé!');
        }
    }
     ...
     */
    hideLoader() {
        console.log('📊 [RepairModal] hideLoader appelé');
        if (!this.elements.loader) {
            this.elements.loader = document.getElementById('repairDetailsLoader');
            console.log('🔍 [RepairModal] Loader trouvé dans hideLoader:', !!this.elements.loader);
        }
        if (!this.elements.detailsContainer) {
            this.elements.detailsContainer = document.getElementById('repairDetailsContent');
            console.log('🔍 [RepairModal] Content trouvé dans hideLoader:', !!this.elements.detailsContainer);
        }
        if (this.elements.loader) {
            this.elements.loader.style.display = 'none';
            console.log('✅ [RepairModal] Loader caché');
        } else {
            console.error('❌ [RepairModal] Loader non trouvé dans hideLoader!');
        }
        if (this.elements.detailsContainer) {
            this.elements.detailsContainer.style.display = 'block';
            console.log('✅ [RepairModal] Content affiché');
        } else {
            console.error('❌ [RepairModal] Content non trouvé dans hideLoader!');
        }
    }
