...
class ModernStatusModal {
    ...
    async openModal(repairId, categoryId, repairData = null) {
        ...
        console.log('📋 Paramètres reçus:', { repairId, categoryId, repairData });
     ...
     */
    async onModalShown() {
        console.log('🔄 Modal affiché, vérification des données...');
        
        // Si on n'a pas d'ID de réparation, essayer de le récupérer depuis les variables globales
        if (!this.currentRepairId) {
            if (window.currentRepairIdForStatus) {
                console.log('🔄 Récupération de l\'ID depuis currentRepairIdForStatus:', window.currentRepairIdForStatus);
                this.currentRepairId = window.currentRepairIdForStatus;
                
                // Mettre à jour les champs cachés
                if (this.elements.repairIdInput) {
                    this.elements.repairIdInput.value = this.currentRepairId;
                }
                ...
                this.showError('Aucune réparation sélectionnée');
                return;
            }
        }
    ...
    bindStatusEvents() {
        ...
        console.log('🔗 Liaison des événements pour', statusButtons.length, 'boutons de statut');
        
        statusButtons.forEach((button, index) => {
            console.log(`🔘 Bouton ${index + 1}:`, {
                statusId: button.dataset.statusId,
                statusCode: button.dataset.statusCode,
            ...
            button.addEventListener('click', (event) => {
                ...
                console.log('🎯 Clic détecté sur le bouton de statut');
                
                const statusId = button.dataset.statusId;
                const statusCode = button.dataset.statusCode;
                
                console.log('📋 Données du statut cliqué:', { statusId, statusCode });
                
                if (!statusId) {
                    console.error('❌ statusId manquant sur le bouton');
                    return;
                }
        ...
        if (statusButtons.length === 0) {
            console.warn('⚠️ Aucun bouton de statut trouvé pour lier les événements');
        }
    }
    ...
    async selectStatus(statusId, statusCode, buttonElement) {
        console.log('🚀 DÉBUT selectStatus:', { statusId, statusCode, buttonElement });
    ...
    async confirmStatusChange() {
        ...
        try {
            ...
            console.log('📋 Données de base ajoutées:', {
            ...
            console.log('📡 Envoi de la requête vers ajax/update_repair_status.php...');
            ...
            if (result.success) {
                ...
                console.log('📋 Données de réponse:', result.data);
                
                // Afficher les informations SMS si disponibles
                if (result.data && result.data.sms_sent !== undefined) {
                    if (result.data.sms_sent) {
                        console.log('📱 SMS envoyé avec succès:', result.data.sms_message);
                    } else {
                        console.log('📱 SMS non envoyé:', result.data.sms_message);
                    }
                }
                ...
                if (result.data && result.data.sms_sent) {
                    message += ' - SMS envoyé';
                } else if (result.data && result.data.send_sms && !result.data.sms_sent) {
                    message += ' - SMS non envoyé: ' + (result.data.sms_message || 'Erreur inconnue');
                }
            ...
            // Restaurer le bouton (mais il n'existe plus dans le nouveau système)
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🔄 Initialisation de ModernStatusModal...');
    
    try {
        window.modernStatusModal = new ModernStatusModal();
        console.log('✅ ModernStatusModal disponible globalement');
        console.log('🔍 Modal instance:', window.modernStatusModal);
    } catch (error) {
        console.error('❌ Erreur lors de l\'initialisation de ModernStatusModal:', error);
    }
});
...
window.openChooseStatusModal = function(repairId, categoryId, repairData = null) {
    ...
    console.log('📋 [COMPAT] Paramètres reçus:', { repairId, categoryId, repairData });
    
    // Vérifier si on a un ID de réparation global
    if (!repairId && window.currentRepairIdForStatus) {
        repairId = window.currentRepairIdForStatus;
        console.log('🔄 [COMPAT] Utilisation de currentRepairIdForStatus:', repairId);
    }
...
// Fonction alternative pour l'ancien système qui utilise une variable globale
window.openStatusModalForRepair = function(repairId) {
    console.log('🔄 [COMPAT] Ouverture via openStatusModalForRepair:', repairId);
    window.openChooseStatusModal(repairId, null, null);
};
