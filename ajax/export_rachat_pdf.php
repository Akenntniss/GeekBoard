<?php
// Attestation de rachat - Interface moderne optimisée impression
...
try {
    ...
    try {
        $stmt = $pdo->prepare('SELECT cle, valeur FROM parametres WHERE cle IN ("company_name", "company_phone", "company_email", "company_address", "company_logo")');
        $stmt->execute();
        $params = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
        
        if ($params) {
            $company_settings = [
                'company_name' => $params['company_name'] ?? 'GEEKBOARD',
                'company_phone' => $params['company_phone'] ?? '',
                'company_email' => $params['company_email'] ?? '',
                'company_address' => $params['company_address'] ?? '',
                'company_hours' => '',
                'company_number' => '' // Le SIRET n'est pas dans parametres, essayons company_settings
            ];
            
            // Essayer de récupérer le SIRET depuis company_settings si disponible
            try {
                $stmt2 = $pdo->prepare('SELECT company_number FROM company_settings LIMIT 1');
                $stmt2->execute();
                $siret_row = $stmt2->fetch(PDO::FETCH_ASSOC);
                if ($siret_row && !empty($siret_row['company_number'])) {
                    $company_settings['company_number'] = $siret_row['company_number'];
                }
            } catch (PDOException $e) {
                // Table company_settings n'existe peut-être pas
            }
        }
    } catch (PDOException $e) {
...
<head>
    ...
    <title>Attestation de Rachat #' . $rachat_id . ' - GeekBoard</title>
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <div class="footer-content" style="display: flex; justify-content: center; align-items: center; gap: 2mm; flex-wrap: wrap;">
                <span class="footer-info" style="font-weight: 700;">🏢 ' . htmlspecialchars($company_settings['company_name'] ?: 'GEEKBOARD') . '</span>
                ' . (!empty($company_settings['company_address']) ? '<span class="footer-separator">•</span><span class="footer-info">📍 ' . htmlspecialchars($company_settings['company_address']) . '</span>' : '') . '
                ' . (!empty($company_settings['company_phone']) ? '<span class="footer-separator">•</span><span class="footer-info">📞 ' . htmlspecialchars($company_settings['company_phone']) . '</span>' : '') . '
                ' . (!empty($company_settings['company_email']) ? '<span class="footer-separator">•</span><span class="footer-info">✉️ ' . htmlspecialchars($company_settings['company_email']) . '</span>' : '') . '
                ' . (!empty($company_settings['company_number']) ? '<span class="footer-separator">•</span><span class="footer-info">🏛️ SIRET: ' . htmlspecialchars($company_settings['company_number']) . '</span>' : '') . '
            </div>
        </div>
    </div>
</body>
