...
try {
    // Essayer de récupérer la connexion via le système standard
    $shop_pdo = null;
    if (function_exists('getShopDBConnection')) {
        $shop_pdo = getShopDBConnection();
    }
    
    // Fallback vers mkmkmk si pas de connexion magasin
    if (!$shop_pdo) {
        $shop_pdo = new PDO(
...
if (window.matchMedia) {
    ...
    mediaQuery.addListener(function(e) {
        ...
        if (localStorage.getItem('theme') === null) {
            if (e.matches) {
                document.body.classList.add('night-mode');
                // Passage automatique en mode nuit
            } else {
                document.body.classList.remove('night-mode');
                // Passage automatique en mode jour
            }
        }
    });
}
...
function resetAllFilters() {
    ...
    // Activer la carte "Suivi des activités en cours" par défaut
    const ongoingCard = document.querySelector('[data-filter="ongoing"]');
    if (ongoingCard) {
        ongoingCard.classList.add('active');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    currentFilters.logType = document.getElementById('logTypeFilter').value;
    ...
    // Initialiser la carte "Suivi des activités en cours" comme active par défaut
    const ongoingCard = document.querySelector('[data-filter="ongoing"]');
    if (ongoingCard) {
        ongoingCard.classList.add('active');
        // Afficher les activités en cours par défaut
        setTimeout(() => showOngoingActivitiesByEmployee(), 100);
    }
    
    // Moniteur d'activité initialisé
    
    // Animation d'entrée pour les cartes de logs
    const logCards = document.querySelectorAll('.modern-log-card');
