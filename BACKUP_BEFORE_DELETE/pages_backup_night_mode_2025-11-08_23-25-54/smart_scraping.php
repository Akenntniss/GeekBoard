...
// Fonction de scraping intelligent avec filtres
function smartScrape($query, $apiKey, $searchEngineId) {
    // 1. Trouver les sites via Google
    $sites = findSitesViaGoogle($query, $apiKey, $searchEngineId);
    ...
    // 2. Scraper chaque site avec filtres intelligents
    foreach ($sites as $site) {
        $sitePrices = scrapeSiteWithFilters($site['url'], $query);
        if (!empty($sitePrices)) {
...
// Fonction pour trouver les sites via Google
function findSitesViaGoogle($query, $apiKey, $searchEngineId) {
    // Nettoyer la requête
...
function scrapeSiteWithFilters($url, $query) {
    $context = stream_context_create([
        'http' => [
            'timeout' => 10,
            'user_agent' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    ...
    // Patterns à éviter (réductions, économies, "-" pour -30€)
    $avoidPatterns = [
...
try {
    $result = smartScrape($data['query'], $data['api_key'], $data['search_engine_id']);
