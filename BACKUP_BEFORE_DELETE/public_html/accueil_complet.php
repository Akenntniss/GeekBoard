...
<!-- CORRECTION FINALE BOUTONS NAVBAR - PRIORITÉ ABSOLUE -->
<link href="assets/css/navbar-buttons-final-fix.css" rel="stylesheet">
                ...
                <div class="tab-content active" id="toutes-taches">
                    <div class="modern-table-container">
                        <div class="modern-table-header">
                            ...
                            <span><?php echo count(get_toutes_taches_en_cours(10)); ?> tâches</span>
                        </div>
                        <div class="modern-table-body">
                            <div class="modern-table-row header">
                                <div class="modern-table-cell title">Titre</div>
                                <div class="modern-table-cell priority">Priorité</div>
                            </div>
                            <?php
                            $toutes_taches = get_toutes_taches_en_cours(10);
                            if (!empty($toutes_taches)) :
                                foreach ($toutes_taches as $tache) :
                                    $priority_class = 'priority-' . strtolower($tache['urgence']);
                            ?>
                                <div class="modern-table-row" data-task-id="<?php echo $tache['id']; ?>" onclick="afficherDetailsTache(event, <?php echo $tache['id']; ?>)">
                                    <div class="modern-table-cell title"><?php echo htmlspecialchars($tache['titre']); ?></div>
                                    <div class="modern-table-cell priority">
                                        <span class="modern-badge <?php echo $priority_class; ?>"><?php echo htmlspecialchars($tache['urgence']); ?></span>
                                    </div>
                                </div>
                            <?php
                                endforeach;
                            ...
                            ?>
                                <div class="modern-table-empty">Aucune tâche en cours</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="tab-content" id="mes-taches">
                    <div class="modern-table-container">
                        <div class="modern-table-header">
                            ...
                            <span><?php echo count(get_taches_en_cours(10)); ?> tâches</span>
                        </div>
                        <div class="modern-table-body">
                            <div class="modern-table-row header">
                                <div class="modern-table-cell title">Titre</div>
            ...
            <div class="table-container">
                <div class="modern-table-container">
                    <div class="modern-table-header">
                        ...
                        <span><?php echo count($reparations_recentes); ?> réparations</span>
                    </div>
                    <div class="modern-table-body">
                        <div class="modern-table-row header">
                            <div class="modern-table-cell title">Client</div>
                            <div class="modern-table-cell priority">Modèle</div>
                        </div>
                        <?php if (count($reparations_recentes) > 0): ?>
                            <?php foreach ($reparations_recentes as $reparation): ?>
                                <div class="modern-table-row" onclick="window.location.href='index.php?page=statut_rapide&id=<?php echo $reparation['id']; ?>'">
                                    <div class="modern-table-cell title">
                                        <i class="fas fa-user me-2" style="color: #4361ee;"></i>
                                        <?php echo htmlspecialchars($reparation['client_nom'] ?? 'N/A'); ?>
                                    </div>
                                    <div class="modern-table-cell priority">
                                        <span class="modern-badge status-en-attente"><?php echo htmlspecialchars($reparation['modele'] ?? ''); ?></span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="modern-table-empty">Aucune réparation récente</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="table-container">
                <div class="modern-table-container">
                    <div class="modern-table-header">
                        ...
                        <span><?php echo count($commandes_recentes); ?> commandes</span>
                    </div>
                    <div class="modern-table-body">
                        <div class="modern-table-row header">
                            <div class="modern-table-cell title">Pièce</div>
                            <div class="modern-table-cell priority">Statut</div>
                        </div>
                        <?php if (count($commandes_recentes) > 0): ?>
                            <?php foreach ($commandes_recentes as $commande): ?>
                                <div class="modern-table-row">
                                    <div class="modern-table-cell title" title="<?php echo htmlspecialchars($commande['nom_piece']); ?>">
                                        <i class="fas fa-box me-2" style="color: #4361ee;"></i>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="modern-table-empty">Aucune commande récente</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Les nouveaux tableaux modernes n'ont plus besoin de JavaScript pour gérer les survols
    // Le CSS gère maintenant complètement l'apparence en mode nuit
});
</script>
...
/* ======================== TABLEAUX MODERNES PERSONNALISÉS ======================== */

/* Container principal pour les tableaux modernes */
...
/* Corps du tableau moderne - utilise CSS Grid */
.modern-table-body {
    display: flex;
...
/* Cellule de la priorité */
.modern-table-cell.priority {
    display: flex;
    align-items: center;
...
/* ======================== MODE NUIT POUR TABLEAUX MODERNES ======================== */
...
.dark-mode .modern-table-header {
    background: linear-gradient(135deg, #0ea5e9, #3b82f6);
}

/* Mode nuit - En-tête des colonnes */
[data-theme="dark"] .modern-table-row.header,
.dark-mode .modern-table-row.header {
    background: rgba(30, 41, 59, 0.8);
    color: #e2e8f0;
    border-bottom-color: rgba(255, 255, 255, 0.1);
}

/* Mode nuit - Lignes de données */
[data-theme="dark"] .modern-table-row,
.dark-mode .modern-table-row {
    background: rgba(30, 41, 59, 0.8) !important;
    border-bottom-color: rgba(255, 255, 255, 0.05);
}

/* Mode nuit - Cellules de titre */
[data-theme="dark"] .modern-table-cell.title,
.dark-mode .modern-table-cell.title {
    color: #f1f5f9;
}

/* Mode nuit - État vide */
[data-theme="dark"] .modern-table-empty,
.dark-mode .modern-table-empty {
    color: #94a3b8;
}
...
.modern-badge.priority-haute {
    background: linear-gradient(135deg, #ef4444, #dc2626);
    color: white;
    box-shadow: 0 2px 8px rgba(239, 68, 68, 0.3);
}

.modern-badge.priority-moyenne {
    background: linear-gradient(135deg, #f59e0b, #d97706);
    color: white;
    box-shadow: 0 2px 8px rgba(245, 158, 11, 0.3);
}

.modern-badge.priority-basse {
    background: linear-gradient(135deg, #06b6d4, #0891b2);
    color: white;
    box-shadow: 0 2px 8px rgba(6, 182, 212, 0.3);
}

.modern-badge.status-en-attente {
    background: linear-gradient(135deg, #f59e0b, #d97706);
    color: white;
    box-shadow: 0 2px 8px rgba(245, 158, 11, 0.3);
}
