<?php
// Script d'export simplifié sans configuration complexe
// Configuration de session pour compatibilité
...
// Debug complet avec header pour debugging
header('X-Debug-Session-ID: ' . session_id());
header('X-Debug-User-ID: ' . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'NONE'));

// Créer un fichier de log séparé pour debug
$debug_file = '/tmp/export_debug.log';
file_put_contents($debug_file, "=== DEBUG EXPORT " . date('Y-m-d H:i:s') . " ===\n", FILE_APPEND);
...
file_put_contents($debug_file, "Cookies: " . print_r($_COOKIE, true) . "\n", FILE_APPEND);

// Vérifier la session utilisateur
if (!isset($_SESSION['user_id'])) {
    error_log("ERREUR: Pas d'user_id en session");
    http_response_code(401);
    exit('Non autorisé - session manquante');
}

error_log("User ID trouvé: " . $_SESSION['user_id']);
...
try {
    ...
    error_log("Produits trouvés: " . count($produits));
} catch (PDOException $e) {
...
error_log("Export terminé avec succès");
?>
