                ...
                // Récupérer la liste des employés pour la frise
                $employees_timeline_query = "SELECT id, full_name, username FROM users WHERE role IN ('admin', 'employee', 'technicien') ORDER BY full_name";
                $employees_timeline_stmt = $shop_pdo->prepare($employees_timeline_query);
                $employees_timeline_stmt->execute();
                $employees_timeline = $employees_timeline_stmt->fetchAll(PDO::FETCH_ASSOC);
                
                // Si aucun employé sélectionné, prendre le premier de la liste
                if ($selected_employee_timeline == 0 && !empty($employees_timeline)) {
                    $selected_employee_timeline = $employees_timeline[0]['id'];
                }
                ...
                if ($selected_employee_timeline > 0) {
                    // Données de pointage
                    $time_query = "SELECT * FROM time_tracking WHERE DATE(clock_in) = ? AND user_id = ? ORDER BY clock_in ASC";
                    $time_stmt = $shop_pdo->prepare($time_query);
                    $time_stmt->execute([$selected_date_timeline, $selected_employee_timeline]);
                    $time_tracking_data = $time_stmt->fetchAll(PDO::FETCH_ASSOC);
                    
                    // Logs de réparations
                    $repair_query = "
                        SELECT rl.*, r.reference, r.type_appareil, CONCAT(c.nom, ' ', c.prenom) as client_nom
                        FROM reparation_logs rl
                        JOIN reparations r ON rl.reparation_id = r.id
                        JOIN clients c ON r.client_id = c.id
                        WHERE DATE(rl.date_action) = ? AND rl.employe_id = ?
                        ORDER BY rl.date_action ASC
                    ";
                    $repair_stmt = $shop_pdo->prepare($repair_query);
                    $repair_stmt->execute([$selected_date_timeline, $selected_employee_timeline]);
                    $repair_logs_data = $repair_stmt->fetchAll(PDO::FETCH_ASSOC);
                }
                
                // Obtenir l'employé sélectionné
                $selected_employee_data = null;
                foreach ($employees_timeline as $emp) {
                    if ($emp['id'] == $selected_employee_timeline) {
