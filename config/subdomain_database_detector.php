...
class SubdomainDatabaseDetector {
    ...
    private $main_config = [
        ...
        '21' => 'geekboard_21',
        'bhnjkiuytgfvb' => 'geekboard_bhnjkiuytgfvb',
        'bra' => 'geekboard_bra',
        'broko' => 'geekboard_broko',
        'dsjdsj' => 'geekboard_dsjdsj',
        'finaltest3' => 'geekboard_finaltest3',
        'hnjkiuyhgt' => 'geekboard_hnjkiuyhgt',
        'jokoloko' => 'geekboard_jokoloko',
        'jouikl' => 'geekboard_jouikl',
        'kedj' => 'geekboard_kedj',
        'kijuhg' => 'geekboard_kijuhg',
        'kijuhygf' => 'geekboard_kijuhygf',
        'kjhg' => 'geekboard_kjhg',
        'kjhkjfr' => 'geekboard_kjhkjfr',
        'kjhyg' => 'geekboard_kjhyg',
        'lamouche' => 'geekboard_lamouche',
        'lkjhg' => 'geekboard_lkjhg',
        'mkmkmk' => 'geekboard_mkmkmk',
        'phoneetoile' => 'geekboard_phoneetoile',
        'phonesystem' => 'geekboard_phonesystem',
        'servir' => 'geekboard_servir',
        'servossl9' => 'geekboard_servossl9',
        'test11' => 'geekboard_test11',
        'test12' => 'geekboard_test12',
        'test55' => 'geekboard_test55',
        'test65' => 'geekboard_test65',
        'test76' => 'geekboard_test76',
        'testfinal09' => 'geekboard_testfinal09',
        'testfinal4' => 'geekboard_testfinal4',
        'testfinal6' => 'geekboard_testfinal6',
        'testfinal7' => 'geekboard_testfinal7',
        'testfinal78' => 'geekboard_testfinal78',
        'testfinal98' => 'geekboard_testfinal98',
        'testfinal99' => 'geekboard_testfinal99',
        'testloko12' => 'geekboard_testloko1',
        'testpointage' => 'geekboard_testpointage',
        'testrouter1' => 'geekboard_testrouter1',
        'testrouter2' => 'geekboard_testrouter2',
        'testsal' => 'geekboard_testsal',
        'testssl1' => 'geekboard_testssl1',
        'testssl10' => 'geekboard_testssl10',
        'testssl14' => 'geekboard_testssl14',
        'testssl16' => 'geekboard_testssl16',
        'testssl2' => 'geekboard_testssl2',
        'testssl3' => 'geekboard_testssl3',
        'testssl4' => 'geekboard_testssl4',
        'testssl5' => 'geekboard_testssl5',
        'testssl6' => 'geekboard_testssl6',
        'testssl7' => 'geekboard_testssl7',
        'testssl8' => 'geekboard_testssl8',
        'testssl88' => 'geekboard_testssl88',
        'testssl98' => 'geekboard_testssl98',
        'testssl99' => 'geekboard_testssl99',
        'touit' => 'geekboard_touit',
        'ujhgv' => 'geekboard_ujhgv',
        'zebilamcj' => 'geekboard_zebilamcj',
    ...
    public function testAllConnections() {
        ...
        foreach ($this->subdomain_mappings as $subdomain => $db_name) {
            try {
                ...
                // Forcer le fuseau horaire MySQL à GMT+1
    ...
    /**
     * Synchroniser les mappings statiques avec la base de données
     */
    public function syncStaticMappings() {
        try {
            $dsn = "mysql:host={$this->main_config['host']};port={$this->main_config['port']};dbname={$this->main_config['main_db']};charset=utf8mb4";
            $pdo = new PDO($dsn, $this->main_config['user'], $this->main_config['pass'], [
                PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
            ...
            // Construire le nouveau tableau de mappings avec ordre spécifique
            $new_mappings = [];
            
            // D'abord les magasins actifs, triés par sous-domaine
            $shop_mappings = [];
            foreach ($shops as $shop) {
                $shop_mappings[$shop['subdomain']] = $shop['db_name'];
            }
            ksort($shop_mappings); // Trier par sous-domaine
            
            // Ajouter les mappings des magasins
            foreach ($shop_mappings as $subdomain => $db_name) {
                $new_mappings[$subdomain] = $db_name;
            }
            
            // Ajouter les mappings par défaut à la fin
            $new_mappings['mdgeek'] = 'geekboard_general';
            $new_mappings['www'] = 'geekboard_general';
            $new_mappings[''] = 'geekboard_general';
            
            // Lire le fichier actuel
            $file_path = __FILE__;
            $content = file_get_contents($file_path);
            if ($content === false) {
                $this->logDebug("Erreur: Impossible de lire le fichier de configuration");
                return false;
            }
            
            // Construire le nouveau tableau sous forme de string
            $mappings_string = "    private \$subdomain_mappings = [\n";
            foreach ($new_mappings as $subdomain => $db_name) {
                if ($subdomain === '') {
                    $mappings_string .= "        '' => 'geekboard_general' // domaine principal sans sous-domaine\n";
                } else {
                    $mappings_string .= "        '$subdomain' => '$db_name',\n";
                }
            }
            $mappings_string .= "    ];";
            
            // Remplacer le tableau existant
            $pattern = '/private\s+\$subdomain_mappings\s*=\s*\[.*?\];/s';
            $new_content = preg_replace($pattern, $mappings_string, $content);
            
            if ($new_content === null || $new_content === $content) {
                $this->logDebug("Erreur: Impossible de mettre à jour les mappings statiques");
                return false;
            }
            
            // Écrire le fichier modifié
            if (file_put_contents($file_path, $new_content) === false) {
                $this->logDebug("Erreur: Impossible d'écrire le fichier de configuration");
                return false;
            }
            
            $this->logDebug("Mappings statiques synchronisés avec succès - " . count($shops) . " magasins");
            return true;
            
        } catch (PDOException $e) {
            $this->logDebug("Erreur lors de la synchronisation des mappings: " . $e->getMessage());
            return false;
        }
    }
...
/**
 * Fonction helper pour synchroniser les mappings statiques
 */
function syncSubdomainMappings() {
    global $subdomain_detector;
    return $subdomain_detector->syncStaticMappings();
}
?> 