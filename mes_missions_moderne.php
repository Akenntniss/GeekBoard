...
// Récupération du solde de la cagnotte
$solde_cagnotte = 0.00;
$solde_points = 0;
try {
    $stmt = $shop_pdo->prepare("
    ...
    ");
    $stmt->execute([$user_id]);
    $cagnotte = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($cagnotte) {
        $solde_cagnotte = (float)$cagnotte['solde_euros'];
        $solde_points = (int)$cagnotte['solde_points'];
    }
} catch (PDOException $e) {
    error_log("Erreur récupération cagnotte: " . $e->getMessage());
}
...
/* Carte cagnotte avec bouton de retrait */
.cagnotte-card {
    position: relative;
}
...
body.night-mode .withdrawal-btn {
    background: linear-gradient(135deg, #10b981, #059669);
    box-shadow: 0 4px 15px rgba(16, 185, 129, 0.3);
}

body.night-mode .withdrawal-btn:hover {
    box-shadow: 0 8px 25px rgba(16, 185, 129, 0.5);
}

/* Info de retrait dans le modal */
.withdrawal-info {
...
.withdrawal-info .info-item i {
    color: #10b981;
    font-size: 1.2rem;
}

body.night-mode .withdrawal-info {
    background: rgba(16, 185, 129, 0.15);
    border-color: rgba(16, 185, 129, 0.3);
}

body.night-mode .withdrawal-info .info-item {
...
<div class="modern-dashboard bg-animated" id="dashboard">
    ...
    <div class="modern-stats-grid fade-in">
        ...
        <div class="modern-stat-card cagnotte-card">
            <div class="stat-header">
                <div class="stat-icon" style="background: linear-gradient(135deg, #10b981, #059669);">
                    <i class="fas fa-wallet"></i>
                </div>
            </div>
            <div class="stat-value"><?= number_format($stats['solde_cagnotte'], 2) ?>€</div>
            <div class="stat-label">Solde Cagnotte</div>
            <?php if ($stats['solde_cagnotte'] > 0): ?>
                <button class="withdrawal-btn" onclick="openWithdrawalModal()">
...
<div class="modern-modal" id="withdrawalModal">
    <div class="modern-modal-dialog">
        <div class="modern-modal-header">
            <h3 class="modern-modal-title">
                <i class="fas fa-money-bill-wave"></i>
        ...
        <div class="modern-modal-body">
            <div class="withdrawal-info">
                <div class="info-item">
                    <i class="fas fa-wallet"></i>
                    <span>Solde disponible: <strong><?= number_format($stats['solde_cagnotte'], 2) ?>€</strong></span>
                </div>
            </div>
            
            <form id="withdrawalForm">
                <div class="modern-form-group">
                    <label for="montant_retrait">
                        <i class="fas fa-euro-sign"></i>
                        Montant à retirer (€)
                    </label>
                    <input 
                        ...
                        step="0.01" 
                        min="0.01" 
                        max="<?= $stats['solde_cagnotte'] ?>"
                        required
                        placeholder="0.00"
                    >
                    <small>Maximum: <?= number_format($stats['solde_cagnotte'], 2) ?>€</small>
                </div>
                ...
                <div class="modern-form-group">
                    ...
                    <textarea 
                        ...
                        rows="4" 
                        required
                        placeholder="Indiquez vos coordonnées bancaires, email PayPal, ou autres informations nécessaires..."
                    ></textarea>
                    <small>Pour virement: IBAN, nom du compte<br>Pour PayPal: adresse email PayPal<br>Pour espèces: date et lieu de retrait souhaité</small>
                </div>
                
                <div class="modern-form-actions">
                    <button type="button" class="modern-btn modern-btn--secondary" onclick="closeModal('withdrawalModal')">
                        <i class="fas fa-times"></i>
                        Annuler
                    </button>
                    <button type="submit" class="modern-btn modern-btn--primary">
...
document.getElementById('withdrawalForm').addEventListener('submit', async function(e) {
    ...
    const formData = {
        montant: parseFloat(document.getElementById('montant_retrait').value),
        methode_paiement: document.getElementById('methode_paiement').value,
        details_paiement: document.getElementById('details_paiement').value
    };
    ...
    try {
        const response = await fetch('ajax/submit_withdrawal_request.php', {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            alert('Demande de retrait soumise avec succès ! Elle sera traitée par un administrateur.');
            closeModal('withdrawalModal');
            location.reload(); // Recharger pour mettre à jour le solde
        } else {
