...
try {
    ...
    foreach ($shops as $shop) {
        try {
            ...
            // Récupérer les templates SMS spécifiques
            $stmt = $shop_pdo->prepare("
                SELECT * FROM sms_templates 
                WHERE type = 'devis' AND est_actif = 1 AND code IN ('devis_rappel', 'devis_expire_gardiennage')
                ORDER BY code
            ");
            $stmt->execute();
            ...
            if (!isset($templates['devis_rappel'])) {
                $templates['devis_rappel'] = [
                    'contenu' => 'Bonjour {client_nom}, votre devis #{devis_numero} expire dans {jours_restants} jours. Consultez-le : {lien_devis}'
                ];
            }
            
            if (!isset($templates['devis_expire_gardiennage'])) {
                $templates['devis_expire_gardiennage'] = [
                    'contenu' => 'Bonjour {client_nom}, votre devis #{devis_numero} a expiré. Gardiennage facturé {prix_gardiennage}€/jour. Consultez : {lien_devis}'
                ];
            }
            ...
            foreach ($devis_a_relancer as $devis) {
                try {
                    ...
                    try {
                        ...
                        logMessage("Erreur lors de la récupération des paramètres d'entreprise: " . $e->getMessage());
                    }
                    
                    // Préparer les variables pour le template (format accolades {variable})
                    $variables_accolades = [
                        ...
                        'prix_gardiennage' => 5, // Prix par défaut du gardiennage
                        'company_name' => $company_name,
                        'company_phone' => $company_phone
                    ];
                    
                    // Préparer les variables pour le template (format crochets [VARIABLE])
                    $variables_crochets = [
                        '[CLIENT_NOM]' => $devis['client_nom'],
                    ...
                    // Remplacer d'abord les variables avec accolades {variable}
                    foreach ($variables_accolades as $key => $value) {
                        $message = str_replace('{' . $key . '}', $value, $message);
                    }
                    
                    // Puis remplacer les variables avec crochets [VARIABLE]
                    foreach ($variables_crochets as $key => $value) {
                        $message = str_replace($key, $value, $message);
                    }
