 ...
 * Version accessible à tous les utilisateurs authentifiés
 */
...
class TimeTrackingAPI {
    ...
    public function __construct() {
        try {
            ...
            // Si pas d'utilisateur connecté, récupérer le premier utilisateur disponible de cette boutique
    ...
    private function checkTimeSlotApproval($user_id, $clock_time) {
        ...
        // 3. Aucun créneau défini = demande approbation
        return [
            'auto_approved' => false,
            'approval_reason' => "Aucun créneau défini pour cette période"
        ];
    }
    ...
    public function handleRequest() {
        ...
        try {
            switch ($action) {
                // Actions utilisateur normales - authentification désactivée temporairement
                case 'clock_in':
                case 'clock_out':
                case 'start_break':
                case 'end_break':
                ...
                case 'get_weekly_report':
                    // L'utilisateur est déjà défini dans le constructeur
                    return $this->handleUserAction($action);
                    
                // Actions admin - nécessitent les droits admin
                case 'admin_get_active':
                case 'admin_approve':
                    // L'utilisateur est déjà défini dans le constructeur
                    if (!$this->is_admin) {
                        // Forcer admin pour les tests
                        $this->is_admin = true;
                    }
    ...
    private function startBreak() {
        ...
        // Vérifier s'il n'y a pas déjà une pause active
        $stmt = $this->pdo->prepare("SELECT id FROM break_sessions WHERE time_tracking_id = ? AND break_end IS NULL");
        ...
        // Créer une nouvelle session de pause
        $stmt = $this->pdo->prepare("INSERT INTO break_sessions (time_tracking_id, break_start, created_at) VALUES (?, NOW(), NOW())");
        $stmt->execute([$active_session['id']]);
        
        // Mettre à jour le statut
        $stmt = $this->pdo->prepare("UPDATE time_tracking SET status = 'break' WHERE id = ?");
        $stmt->execute([$active_session['id']]);
        
        $this->sendResponse(true, 'Pause commencée', 200, [
            'break_id' => $this->pdo->lastInsertId(),
    ...
    private function endBreak() {
        // Trouver la session active
        $stmt = $this->pdo->prepare("SELECT id FROM time_tracking WHERE user_id = ? AND status = 'break' ORDER BY clock_in DESC LIMIT 1");
        $stmt->execute([$this->current_user_id]);
        $active_session = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $stmt = $this->pdo->prepare("SELECT id, break_start FROM break_sessions WHERE time_tracking_id = ? AND break_end IS NULL ORDER BY break_start DESC LIMIT 1");
        $stmt->execute([$active_session['id']]);
        $active_break = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$active_break) {
            throw new Exception('Aucune session de pause active trouvée');
        }
        
        $break_end_time = date('Y-m-d H:i:s');
        
        // Calculer la durée de la pause
        $break_start_time = new DateTime($active_break['break_start']);
        $break_end_time_obj = new DateTime($break_end_time);
        $duration = $break_end_time_obj->diff($break_start_time);
        $break_duration = $duration->h + ($duration->i / 60) + ($duration->s / 3600);
        
        // Mettre à jour la session de pause
        $stmt = $this->pdo->prepare("UPDATE break_sessions SET break_end = ?, break_duration = ? WHERE id = ?");
        $stmt->execute([$break_end_time, $break_duration, $active_break['id']]);
        
        // Remettre le statut à 'active'
        $stmt = $this->pdo->prepare("UPDATE time_tracking SET status = 'active' WHERE id = ?");
        $stmt->execute([$active_session['id']]);
        
        $this->sendResponse(true, 'Pause terminée', 200, [
    ...
    private function getCurrentStatus() {
        // Récupérer le statut actuel (sans dépendance à break_sessions)
        $stmt = $this->pdo->prepare("
            SELECT tt.*, 
                   CASE 
        ...
        if ($status) {
            $response_data = [
                ...
                'break_start' => null // Pas de gestion des pauses pour l'instant
            ];
        }
    ...
    private function getTodayEntries() {
        $stmt = $this->pdo->prepare("
            SELECT tt.*, 
                   COALESCE(SUM(bs.break_duration), 0) as total_break_duration,
                   COUNT(bs.id) as break_count
            FROM time_tracking tt
            LEFT JOIN break_sessions bs ON tt.id = bs.time_tracking_id
            WHERE tt.user_id = ? 
            AND DATE(tt.clock_in) = CURDATE()
            GROUP BY tt.id
            ORDER BY tt.clock_in DESC
        ");
        $stmt->execute([$this->current_user_id]);
        ...
        $this->sendResponse(true, 'Entrées d\'aujourd\'hui récupérées', 200, $entries);
    }
    ...
    private function approveEntry() {
        ...
        $action = $_POST['approve_action'] ?? 'approve'; // 'approve' ou 'reject'
        $admin_notes = $_POST['admin_notes'] ?? '';
        
        if (!$entry_id) {
            throw new Exception('ID d\'entrée manquant');
        }
        ...
        $stmt = $this->pdo->prepare("
            ...
            WHERE id = ?
        ");
        $stmt->execute([$approved, $admin_notes, $entry_id]);
        
        $message = $approved ? 'Entrée approuvée' : 'Entrée rejetée';
        $this->sendResponse(true, $message, 200, ['entry_id' => $entry_id, 'approved' => $approved]);
    }
    ...
    private function getWeeklyReport() {
        ...
        $this->sendResponse(true, 'Rapport hebdomadaire récupéré', 200, $weekly_data);
    }
