...
// Journaliser le chemin reçu pour le débogage
error_log("DEBUG DEVIS: Chemin reçu = '" . $path . "'");

// Vérifier si c'est une URL de devis public (accès sans authentification)
if (preg_match('/^devis\/([a-f0-9]{32})\/?$/', $path, $matches)) {
    // C'est un devis public, rediriger directement vers devis_client.php
    $lien_securise = $matches[1];
    error_log("DEBUG DEVIS: Redirection vers devis_client.php avec lien = " . $lien_securise);
