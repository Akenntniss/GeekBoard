...
window.RepairModal = window.RepairModal || {
    ...
    initRepairDetailsActions() {
        ...
        document.querySelectorAll('.start-repair-btn, .stop-repair-btn').forEach(btn => {
            btn.addEventListener('click', () => {
    ...
    .then(data => {
        if (data.success) {
            if (data.has_active_repair && data.active_repair.id != repairId) {
                ...
                alert('Erreur lors du démarrage : Vous avez déjà une réparation active (#' + activeRepair.id + '). Veuillez d\'abord la terminer.');
                
                console.log('🚨 Alert fermée, préparation du modal activeRepairModal...');
                console.log('🔍 Données de la réparation active:', activeRepair);
                
                // Après que l'utilisateur clique sur OK, ouvrir le modal activeRepairModal
                setTimeout(() => {
                    console.log('🎯 Début de la configuration du modal activeRepairModal');
                    
                    // Vérifier que les éléments existent
                    const activeRepairIdEl = document.getElementById('activeRepairId');
                    const activeRepairDeviceEl = document.getElementById('activeRepairDevice');
                    const activeRepairClientEl = document.getElementById('activeRepairClient');
                    const activeRepairModalEl = document.getElementById('activeRepairModal');
                    
                    if (!activeRepairModalEl) {
                        console.error('❌ Modal activeRepairModal non trouvé !');
                        return;
                    }
                    
                    // Remplir les informations dans le modal activeRepairModal
                    if (activeRepairIdEl) activeRepairIdEl.textContent = `#${activeRepair.id}`;
                    if (activeRepairDeviceEl) activeRepairDeviceEl.textContent = activeRepair.modele || 'Non renseigné';
                    if (activeRepairClientEl) activeRepairClientEl.textContent = `${activeRepair.client_nom || ''} ${activeRepair.client_prenom || ''}`.trim() || 'Non renseigné';
                    ...
                    console.log('✅ Informations remplies dans le modal');
                    ...
                    setTimeout(() => {
                        console.log('🎪 Ouverture du modal activeRepairModal');
                        ...
                        console.log('🔘 Boutons trouvés:', completeButtons.length);
                        ...
                        completeButtons.forEach(button => {
                            ...
                            newButton.addEventListener("click", function() {
                                ...
                                // Fonction spéciale qui termine la réparation active ET démarre la nouvelle
                                completeActiveRepairAndStartNew(activeRepair.id, repairId, status);
                            });
                        });
                        
                        // Afficher le modal activeRepairModal
                        try {
                            const activeRepairModal = new bootstrap.Modal(activeRepairModalEl);
                            activeRepairModal.show();
                            console.log('🎉 Modal activeRepairModal ouvert avec succès !');
                        } catch (error) {
                            console.error('❌ Erreur lors de l\'ouverture du modal:', error);
                        }
                    }, 500);
                }, 500); // Délai plus long pour que l'alerte se ferme complètement
