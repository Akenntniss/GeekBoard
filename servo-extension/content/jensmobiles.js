/**
 * SERVO Extension - Content Script pour Jensmobiles
...
(function () {
    ...
    // Vérifier si on est sur une page produit Jensmobiles
    function isProductPage() {
        return document.querySelector('h1.fa_designation') !== null ||
            document.querySelector('#id_ajouter_panier_fiche') !== null;
    }

    // Extraire les données du produit
    function extractProductData() {
        const nameEl = document.querySelector('h1.fa_designation') || document.querySelector('h1');
        const priceEl = document.querySelector('span[itemprop="price"]');
        const refEl = document.querySelector('.fa_reference span');

        if (!nameEl) {
            console.log('[SERVO Jensmobiles] Impossible de trouver le nom du produit');
            return null;
        }

        let price = '0';
        if (priceEl) {
            price = priceEl.textContent.replace('€', '').replace('ht', '').replace(/\s/g, '').replace(',', '.').trim();
        }
    ...
    async function searchClients(query) {
        ...
        try {
            ...
            console.error('[SERVO Jensmobiles] Erreur recherche:', error);
            resultsDiv.innerHTML = '<div class="servo-no-results">Erreur de connexion<br><small>Vérifiez l\'URL de votre Dashboard dans les paramètres de l\'extension</small></div>';
        }
    }
    ...
    async function createAndAddClient(nom, prenom, phone, email) {
        ...
        try {
            ...
            console.error('[SERVO Jensmobiles] Erreur création client:', error);
    ...
    async function confirmAddToServo(clientId) {
        ...
        try {
            ...
            console.error('[SERVO Jensmobiles] Erreur:', error);
    ...
    function injectServoButton() {
        ...
        const addToCartBtn = document.querySelector('#id_ajouter_panier_fiche');
        ...
        console.log('[SERVO Jensmobiles] Bouton injecté', productData);
    }
