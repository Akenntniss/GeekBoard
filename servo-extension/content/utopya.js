/**
 * SERVO Extension - Content Script pour Utopya
...
(function () {
    ...
    function isProductPage() {
        return document.querySelector('h1.product.block-title span.base') !== null;
    }

    // Extraire les données du produit
    function extractProductData() {
        const nameEl = document.querySelector('h1.product.block-title span.base');
        const skuEl = document.querySelector('li.attr-sku strong.data');
        ...
        price = price.replace('€', '').replace(/\s/g, '').replace(',', '.').trim();

        return {
            name: nameEl.textContent.trim(),
            reference: skuEl ? skuEl.textContent.trim() : '',
    ...
    function injectServoButton() {
        ...
        // Trouver le conteneur visible
        const allContainers = document.querySelectorAll('.product-atc');
        ...
        if (!visibleContainer) {
            const addToCartBtns = document.querySelectorAll('.action.primary.tocart');
            for (const btn of addToCartBtns) {
                if (btn.offsetWidth > 0 && btn.offsetHeight > 0) {
                    visibleContainer = btn.parentElement;
                    break;
                }
            }
        }
        ...
        if (!visibleContainer) {
            visibleContainer = document.querySelector('.product-info-main');
        }
        ...
        console.log('[SERVO] Bouton injecté', productData);
    }
