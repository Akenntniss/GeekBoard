    ...
    if (taskElement) {
        ...
        if (taskModal) {
            ...
            fetch(`ajax/get_tache_details.php?id=${taskId}`)
                ...
                .then(data => {
                    // Log complet de la reponse pour debug
                    try {
                        console.log('[Task Details] AJAX response:', JSON.parse(JSON.stringify(data)));
                    } catch (_) {
                        console.log('[Task Details] AJAX response (raw):', data);
                    }
                    if (data.success) {
