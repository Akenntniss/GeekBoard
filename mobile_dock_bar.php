...
<style>
/* ====================================================================
   🚫 DÉSACTIVATION COMPLÈTE DES ANIMATIONS - AFFICHAGE IMMÉDIAT
==================================================================== */
...
/* AFFICHAGE IMMÉDIAT DES ÉLÉMENTS CACHÉS */
.links__link {
    opacity: 1 !important;
...
.links {
	...
	--radius: clamp(88px, calc(min(50vw, 50vh) - var(--link-size) / 2 - 12px), 180px);
...
.links__item {
	...
	transform: rotate(var(--rotation)) translate(var(--radius)) rotate(calc(var(--rotation) * -1));
    transition: none !important;
}

.links__item:hover {
	transform: rotate(var(--rotation)) translate(calc(var(--radius) + 5px)) rotate(calc(var(--rotation) * -1)) scale(1.08);
}
...
@media (max-width: 375px) {
    .links {
        --link-size: calc(var(--base-grid) * 13);
    }
}
...
function updateTimeTrackingButtonCircular() {
    ...
    fetch('ajax/get_timetracking_status.php')
        ...
        .then(data => {
            console.log('📊 Statut pointage reçu:', data);
            ...
            if (data.success) {
                if (data.is_clocked_in) {
                    ...
                    console.log('✅ Utilisateur pointé IN - affichage bouton OUT');
...
async function executePointageWithAnimation(pointageType, isCurrentlyClockedIn) {
    try {
        ...
        // Récupérer l'user_id depuis la page si disponible
