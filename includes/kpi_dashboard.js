...
class KPIDashboard {
    ...
    async loadAIProfiles() {
        try {
            ...
            // Délégation d'événements pour les boutons Analyser
            $(document).off('click', '.btn-analyze-profile').on('click', '.btn-analyze-profile', function () {
                const profileId = $(this).data('profile-id');
                const profileName = $(this).data('profile-name');
                const profileDesc = $(this).data('profile-desc');
                openAIAnalysisModal(profileId, profileName, profileDesc);
            });
...
async function launchAIAnalysis() {
    ...
    console.log(`🚀 Récupération du prompt pour profil #${currentAIProfileId}`);

    // Masquer le bouton
    $('#aiLaunchSection').fadeOut(300);

    try {
        // Récupérer les détails du profil incluant le system_prompt
        const response = await fetch(`/ajax/get_ai_profiles.php?id=${currentAIProfileId}`);
        const data = await response.json();
        ...
        if (data.success && data.data) {
            ...
            $('#aiSystemPrompt').text(profile.system_prompt || 'Aucun prompt système défini');

            // Afficher la section du prompt
            $('#aiPromptSection').fadeIn(400);
            console.log('✅ Prompt système affiché');
        } else {
            console.error('❌ Erreur récupération profil:', data.error);
            // Lancer quand même l'analyse
            confirmAndLaunchAnalysis();
        }
...
async function confirmAndLaunchAnalysis() {
    if (!currentAIProfileId) {
        console.error('Aucun profil sélectionné');
        return;
    }

    console.log(`🚀 Lancement analyse IA pour profil #${currentAIProfileId}`);

    // Masquer le prompt, afficher l'animation
    $('#aiPromptSection').fadeOut(300, () => {
    ...
    try {
        ...
        $('.ai-criteria-section input[type="checkbox"]:checked').each(function () {
        ...
        if (selectedCriteria.length === 0) {
            throw new Error('Veuillez sélectionner au moins un critère KPI'); // Changed Exception to Error
        }

        // Récupérer UNIQUEMENT les données KPI sélectionnées
        const kpiData = {};

        for (const criterion of selectedCriteria) {
            console.log(`🔄 Chargement ${criterion}...`);
            kpiData[criterion] = await dashboard.fetchAPI(criterion);
        }

        console.log('✅ Données KPI récupérées:', Object.keys(kpiData));
        ...
        const response = await fetch('/ajax/generate_ai_analysis.php', {
            ...
            body: JSON.stringify({
                ...
                kpi_data: kpiData,
                employee_id: dashboard.currentFilters.employee_id,
        ...
        $('#aiLoadingSection').fadeOut(300, () => {
            if (data.success) {
                ...
                $('#aiAnalysisContent').html(formatted);
